...
Tree.prototype.DFSelect = function (filter) {
  ...
  function tracking(node, depth) {
    if (!!filter(node.value, depth)) results.push(node.value)
    for (var i = 0; i < node.children.length; i++)
      tracking(node.children[i], depth + 1)
  }
  tracking(this, depth)
  return results
};
...
console.log(root1.DFSelect(function (value, depth) { return value % 2 })) // [1, 5, 3, 7]
console.log(root1.DFSelect(function (value, depth) { return depth === 1 })) // [2, 3]
console.log(root1.DFSelect(function (value, depth) { return !!value })) // [2, 3]